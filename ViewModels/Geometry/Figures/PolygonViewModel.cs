// ViewModels/Geometry/Figures/PolygonViewModel.cs
using System;
...
public abstract class PolygonViewModel : FigureViewModel
{
    ...
    protected PolygonViewModel(IEnumerable<Point_1> points, 
        Color lineColor, double thickness, Color fillColor, double opacity)
    {
        Name = GetType().Name.Replace("ViewModel", "");
        
        // Конвертируем Point_1 → PointViewModel для реактивности
    ...
    public override Point_1 Center => new Point_1(
    ...
    public override void Rotate(double angleDegrees)
    {
        ...
        foreach (var vertex in Vertices)
        {
            var rotated = TransformHelpers.RotatePoint(vertex.ToPoint(), center, angleDegrees);
            vertex.X = rotated.X;
            vertex.Y = rotated.Y;
        }
        NotifyPropertyChanged();
    ...
    private bool RayIntersectsSegment(Point_1 p, Point_1 a, Point_1 b, double eps)
    {
        // Упорядочиваем по Y
        if (a.Y > b.Y) (a, b) = (b, a);
    ...
    protected void UpdateVerticesScale(Point_1 center, double scale)
    {
        foreach (var vertex in Vertices)
        {
