...
{
    private static RhombusViewModel Create(double centerX = 100, double centerY = 100, double width = 100, double height = 80)
        => new RhombusViewModel(centerX, centerY, width, height, Color.Black, 1, Color.Transparent, 1.0);

    [Fact]
    public void Constructor_SetsCorrectCenter()
    {
        var r = Create(10, 20);
        Assert.Equal(10, r.CenterX, 10);
        Assert.Equal(20, r.CenterY, 10);
    }
    ...
    public void Move_ShiftsPosition()
    {
        var r = Create(0, 0);
        r.Move(15, 25);
        Assert.Equal(15, r.CenterX, 10);
        Assert.Equal(25, r.CenterY, 10);
    }
...
// ─── RightTriangleViewModel ───────────────────────────────────────────────────

public class RightTriangleViewModelTests
{
    private static RightTriangleViewModel Create(double x = 0, double y = 0, double width = 100, double height = 80)
        => new RightTriangleViewModel(x, y, width, height, Color.Black, 1, Color.Transparent, 1.0);
